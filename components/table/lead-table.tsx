...
export function LeadTable() {
  ...
  const onRemove = async (id: string | string[]) => {
    if (!!removeItemId) return;

    typeof id == "string" && setRemoveItemId(id);

    typeof id != "string" && setIsRemoveAll(true);

    await new Promise((res) => setTimeout(res, 100));

    setRemoveItemId(null);

    typeof id != "string" && setIsRemoveAll(false);
    ...
    typeof id == "string"
      ...
      : toastify(
          "success",
          `${selectedLead.length} lead${selectedLead.length > 1 ? "s" : ""} ${
            selectedLead.length > 1 ? "have" : "has"
  ...
  return (
    <div className="relative">
      ...
      <Table>
            ...
            <TableHead>
              <div className="flex items-center justify-start space-x-2">
                <Checkbox
                  ...
                  checked={selectedLead.length >= count}
                />
                {selectedLead.length > 0 && <p>{selectedLead.length}</p>}
              </div>
            </TableHead>
            ...
            {dataTableHeader.map((dataHeader) => (
              <TableHead key={dataHeader.key}>
                <p className="flex items-center gap-2 text-nowrap">
                  {dataHeader.value}{" "}
                  {dataHeader.canEdit && <Pencil size={13} />}
                </p>
              </TableHead>
            ))}
          </TableRow>
        ...
        <TableBody className={cn(isRemoveAll && "opacity-50")}>
          {(!!filteredLeadData
